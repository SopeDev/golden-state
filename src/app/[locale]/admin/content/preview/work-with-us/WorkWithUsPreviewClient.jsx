...
export default function WorkWithUsPreviewClient() {
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      ...
      if (event.data?.type !== 'WORK_WITH_US_PREVIEW_UPDATE') return
  ...
  useEffect(() => {
    ...
    candidates.forEach((node) => {
      if (node.closest('[data-role-field]')) return
  ...
  const handlePreviewClickCapture = (event) => {
    const interactiveTarget = event.target.closest('a, button, select')
  ...
  const handlePreviewClick = (event) => {
    const roleTarget = event.target.closest('[data-role-field]')
    if (roleTarget) {
      const card = roleTarget.closest('[data-role-id]')
      const roleId = card?.getAttribute('data-role-id')
      const field = roleTarget.getAttribute('data-role-field')
      if (roleId && field) {
        event.preventDefault()
        event.stopPropagation()
        window.parent.postMessage(
          {
            type: 'WORK_WITH_US_PREVIEW_SELECT',
            payload: { kind: 'role', roleId, field, locale },
          },
    ...
    window.parent.postMessage(
      {
        type: 'WORK_WITH_US_PREVIEW_SELECT',
        payload: { kind: 'static', key, locale },
      },
      window.location.origin
    )
  }
  ...
  return (
    <div ref={containerRef} onClickCapture={handlePreviewClickCapture} onClick={handlePreviewClick}>
      <WorkWithUsPage content={content} />
    </div>
  )
}
