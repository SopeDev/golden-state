...
export default function AboutPreviewClient() {
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      ...
      if (event.data?.type !== 'ABOUT_PREVIEW_UPDATE') return
  ...
  useEffect(() => {
    ...
    candidates.forEach((node) => {
      ...
      // Prefer leaf-like nodes so card/list wrappers don't swallow clicks.
  ...
  useEffect(() => {
    ...
    const previousContentMarginTop = content?.style.marginTop || ''
    ...
    return () => {
      ...
      if (content) {
        content.style.marginTop = previousContentMarginTop
      }
      if (footer) {
